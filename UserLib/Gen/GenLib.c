...
__attribute__((alias("USART_iprintf"))) int USART_printf(USART_TypeDef* USART,const char *fmt, ...);
...
int USART_iprintf(USART_TypeDef* USART,const char *fmt, ...)
{
	int length = 0;
	va_list va;
	va_start(va, fmt);
...
void RTC_1s_it_init()
{
	...
	RCC_RTCCLKConfig(RCC_RTCCLKSource_HSE_Div128);	//选择HSE_Div128时钟作为RTC时钟，此外还可以选择：LSI、LSE
...
void CAN1_RX0_IRQHandler()
{
	...
	CanRxMsg RxMessage;
	digitalWriteA(GPIO_Pin_4, LED_FLAG);
	CAN_Receive(CAN1, CAN_FIFO0, &RxMessage);
	Analysis(RxMessage);
	LED_FLAG = !LED_FLAG;
//	USB_printf("%x",RxMessage.ExtId);
}
#endif
