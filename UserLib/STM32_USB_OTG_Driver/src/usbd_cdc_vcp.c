...
{
  receive_count = Len;
  /*TODO CDC设备模拟VCP,数据接收后入队列*/
  inqueue_RX(Rxbuffer, receive_count);
  DCD_EP_PrepareRx(&USB_OTG_dev, CDC_OUT_EP, Rxbuffer, receive_count);//准备下次接收
  return USBD_OK;
}
...
//////////////////////////////////////////////////////////
//uint32_t USB_RxRead(USB_OTG_CORE_HANDLE * pdev,uint8_t *buffter, uint32_t buffterSize)
//{
//	DCD_EP_PrepareRx(pdev, CDC_OUT_EP, pbuf, buf_len);
